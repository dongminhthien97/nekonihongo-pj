// src/components/FlashcardKanji.tsx
...
export function FlashcardKanji({
  onNavigate,
}: {
  onNavigate: (page: string) => void;
}) {
  const [compounds, setCompounds] = useState<KanjiCompound[]>([]);
  const [allCompounds, setAllCompounds] = useState<KanjiCompound[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isFlipped, setIsFlipped] = useState(false);
  const [showEndModal, setShowEndModal] = useState(false);
  const [lessonTitle, setLessonTitle] = useState("Flashcard Kanji");
  ...
  useEffect(() => {
    ...
    const loadFlashcardData = () => {
      const data = localStorage.getItem("nekoFlashcardKanjiData");
      ...
      try {
        ...
        setAllCompounds(parsed.allCompounds || []);
        setLessonTitle(parsed.lessonTitle || "Flashcard Kanji");
  ...
  if (compounds.length === 0) {
    return (
      <NekoAlertModal
        isOpen={true}
        onClose={() => onNavigate("kanji")}
        title="Meow meow..."
        message="Không có từ ghép Kanji để học rồi! Mèo đưa bạn về trang Kanji nhé"
      />
    );
  }
  ...
  const progress =
    compounds.length > 0 ? (((currentIndex % 10) + 1) / 10) * 100 : 0;
  ...
  const handleContinue = () => {
    ...
    const learnedWords = new Set(compounds.map((c) => c.word));
    const remaining = allCompounds.filter((c) => !learnedWords.has(c.word));

    let newCompounds: KanjiCompound[] = [];
    ...
    if (remaining.length >= 10) {
      ...
      newCompounds = [...remaining];
      const needed = 10 - remaining.length;
      const old = allCompounds
        .filter((c) => learnedWords.has(c.word))
      ...
      <main className="container mx-auto px-4 py-12 flex flex-col items-center">
        ...
        <div className="relative w-full max-w-2xl h-96 mb-12 perspective-1000">
          ...
          >
            ...
            <div className="flashcard-front-face">
              <p className="huge-dark-title">{currentCompound.word}</p>
            ...
            <div className="flashcard-back-face">
              <p className="centered-hero-text">{currentCompound.meaning}</p>
              <p className="caption-text-white-subtle">
          ...
          >
            ...
            <div className="flex-centered-text-row">
              <span className="heavy-shadowed-title">
                {currentIndex === compounds.length - 1
                  ? "HOÀN THÀNH!"
                  : "TIẾP THEO"}
              </span>
              {currentIndex !== compounds.length - 1 && (
          ...
          <div className="modal-backdrop-dark">
            <div className="modal-card-large">
              ...
              <p className="paragraph-large-spaced">
                Bạn đã học xong 10 từ ghép Kanji! Mèo tự hào về bạn lắm!
              </p>
              <div className="flex gap-8 justify-center">
                <button
                  onClick={handleContinue}
                  className="gradient-cta-button-large"
                >
                  Học tiếp 10 từ nữa!
                </button>
                <button
                  onClick={() => onNavigate("kanji")}
                  className="gray-cta-button-large"
                >
                  Về trang Kanji
                </button>
              </div>
        ...
        {showLostWordsModal && (
          <NekoAlertModal
            ...
            title="Meow meow..."
            message="Mèo lạc mất danh sách từ rồi! Mèo đưa bạn về trang Kanji nhé..."
          />
        )}
