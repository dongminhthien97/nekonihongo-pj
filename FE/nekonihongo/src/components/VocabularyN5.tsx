...
export function VocabularyN5({
  ...
  const [words, setWords] = useState<N5Word[]>([]);
  ...
  // Tính số ngày cần học (dựa trên data đã tìm kiếm)
  const totalDays = Math.ceil(searchedWords.length / WORDS_PER_DAY);

  // Lấy đúng 25 từ của ngày hiện tại
  const currentDayWords = searchedWords.slice(
    (selectedDay - 1) * WORDS_PER_DAY,
    selectedDay * WORDS_PER_DAY
  );
  ...
  if (isLoading)
    return <NekoLoading message="Mèo đang chuẩn bị từ vựng N5..." />;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="relative z-10 mb-12 md:mb-16">
          ...
          <div className="glass-effect-container">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSelectedDay(1); // reset về ngày 1 khi tìm kiếm
              }}
        ...
        {/* Bảng từ vựng – hiển thị 25 từ của ngày hiện tại */}
            ...
            <tbody className="bg-white">
              {currentDayWords.length === 0 ? (
                ...
                currentDayWords.map((word) => (
                  <tr key={word.stt} className="list-item-hover">
        ...
        {/* MÈO BAY – CLICK HỌC FLASHCARD 10 TỪ CỦA NGÀY HIỆN TẠI */}
